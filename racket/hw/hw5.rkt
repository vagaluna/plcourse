  ...
  (cond [(var? e) 
         ...
         (let ([c (eval-under-env (call-funexp e) env)]
           ...
           (if (closure? c)
               (letrec ([f (closure-fun c)]
                        ...
                        [f-body (fun-body f)]
                        [env1 (extend-env f-param arg f-env)]
                        [env2 (if f-name
                                  (extend-env f-name c env1)
                                  env1)])
        ...
        [(mlet? e)
         (eval-under-env (mlet-body e)
                         (extend-env (mlet-var e) (eval-under-env (mlet-e e) env) env))]
...
(define (mlet* lstlst e2)
  (if (null? lstlst)
      ...
      (let* ([binding (car lstlst)]
             [name (car binding)])
        (mlet name (cdr binding) (mlet* (cdr lstlst) e2)))))
...
(define (compute-free-vars e)
  (define (free-vars e)
    (cond [(var? e) (set (var-string e))]
          ...
          [(fun? e) (set-remove (set-remove (free-vars (fun-body e)) (fun-nameopt e)) (fun-formal e))]
          [(call? e) (set-union (free-vars (call-funexp e)) (free-vars (call-actual e)))]
          [(mlet? e) (set-remove (free-vars (mlet-body e)) (mlet-var e))]
  ...
  (cond [(add? e) (add (compute-free-vars (add-e1 e)) (compute-free-vars (add-e2 e)))]
        ...
        [(fun? e) (fun-challenge (fun-nameopt e) (fun-formal e)
                                 (compute-free-vars (fun-body e)) (free-vars e))]
...
(define (eval-under-env-c e env)
  ...
  (cond [(var? e) 
        ...
        [(fun-challenge? e) (closure (filter (lambda (binding) (set-member? (fun-challenge-freevars e) (car binding))) env) e)]
         ...
         (let ([c (eval-under-env-c (call-funexp e) env)]
           ...
           (if (closure? c)
               (letrec ([f (closure-fun c)]
                        ...
                        [f-param (fun-challenge-formal f)]
        ...
        [(mlet? e)
         (eval-under-env-c (mlet-body e)
                           (extend-env (mlet-var e) (eval-under-env-c (mlet-e e) env) env))]
